...
{
    public partial class EstadisticaProduccion2 : System.Web.UI.Page
    {
        InformeProduccion_Controller ip = new InformeProduccion_Controller();
        Partes pp = new Partes();
        Controller_InformeProduccion_V2 inf = new Controller_InformeProduccion_V2();
                    ...
                    if (ft > fi)
                    {
                        if (ddlMaquina.SelectedValue.ToString() != "" || ddlMaquina.SelectedValue.ToString() != "Seleccione...")
                        {
                            if (ddlMaquina.SelectedValue.ToString() == "MR408")
                            {
                                ...
                                RadGrid2.Visible = false;
                                RadGrid1.DataSource = inf.EstadisticaProduccion_V2("MR408", fi, ft, 0);
                                RadGrid1.DataBind();
                            }
                            else if (ddlMaquina.SelectedValue.ToString() == "C150")
                            {
                                RadGrid1.Visible = true;
                                RadGrid2.Visible = false;
                                RadGrid1.DataSource = inf.EstadisticaProduccion_V2("C150", fi, ft, 0);
                                RadGrid1.DataBind();
                            }
                            else if (ddlMaquina.SelectedValue.ToString() == "KBA" || ddlMaquina.SelectedValue.ToString() == "M2016")
                            {
                                ...
                                RadGrid2.DataSource = inf.EstadisticaProduccion_V2(ddlMaquina.SelectedValue.ToString(), fi, ft, 2);
                                RadGrid2.DataBind();
                            }
                            else
                            {
                                RadGrid1.Visible = false;
                                RadGrid2.Visible = true;
                                RadGrid2.DataSource = inf.EstadisticaProduccion_V2(ddlMaquina.SelectedValue.ToString(), fi, ft, 1);
                                RadGrid2.DataBind();
                            }
            ...
            try
            {
                if (RadGrid1.Visible == true)
                {
                    if (RadGrid1.Items.Count > 0)
                    {
                        ...
                        ExportToExcel("Estadistica_Produccion", "", GridView1);
                    }
                }
                else
                {
        ...
        private void ExportToExcel(string nameReport, string Titulo, GridView wControl)
        {
            ...
            la.Text = "<div align='center'>Estadistica Producción</div><div align='center'>";
