...
{
    public partial class PDFKilosTrans : System.Web.UI.Page
    {
        public static string info = "";
        public string OT = "";
        public string NombreOT = "";
        public string Cliente = "";
        public string Transportados = "";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string Transportados = Request.QueryString["Transportista"];
            string FeInicio = Request.QueryString["FeInicio"];
            string FeTermino = Request.QueryString["FeTermino"];
            VariasPaginas(OT, NombreOT, Cliente, Transportados ,FeInicio, FeTermino);
        }
        ...
        //Se genera el PDF
        public void VariasPaginas(string OT, string NombreOT, string Cliente, string Tranportista, string FeInicio, string FeTermino)
        {
            info = "";
            List<DespachoKilos> lista = ListDespachoKilos(OT, NombreOT, Cliente, Tranportista, FeInicio, FeTermino);
            ...
            if (OT != "" || NombreOT != "" || Cliente != "" || Tranportista!="" || (FeInicio != "" && FeTermino != ""))
            {
                if (OT != "")
                {
                    info = info + "OT : " + OT;
                ...
                if (Tranportista != "") { info = info + "Transportista : " + Tranportista; }
            }
            else
            {
            ...
            Paragraph para = new Paragraph("      Informe Kilos Transportados \n", time);
            ...
            //
            grdTable.Width = 100;
            int[] w = new int[] { 26, 15, 8, 13, 8, 30, 8, 10,10 };
            ...
            int contador = 0;
            foreach (DespachoKilos des in lista)
            {
                if ((contador % 2) == 0)
                {
                ...
                grdTable.AddCell(new Paragraph(des.Transportista, time));
                grdTable.AddCell(new Paragraph(des.Patente, time));
                grdTable.DefaultCell.HorizontalAlignment = Element.ALIGN_RIGHT;
                grdTable.AddCell(new Paragraph(des.Guias.ToString(), time));
                grdTable.AddCell(new Paragraph(des.FechaDespacho, time));
                grdTable.DefaultCell.HorizontalAlignment = Element.ALIGN_LEFT;
                grdTable.AddCell(new Paragraph(des.OT,time));
                grdTable.AddCell(new Paragraph(des.NumeroOT, time));
                grdTable.DefaultCell.HorizontalAlignment = Element.ALIGN_RIGHT;
                grdTable.AddCell(new Paragraph(des.PesoUnitario, time));
                grdTable.AddCell(new Paragraph(des.Cant.ToString("N0"), time));
                grdTable.AddCell(new Paragraph(des.TotalKilos.ToString(),time));
                contador++;
            ...
            Response.AddHeader("Content-Disposition", "attachment;filename=Reporte_KilosTrans.pdf");
        ...
        public class itsEvents : PdfPageEventHelper
        {
            ...
            iTextSharp.text.pdf.PdfDocument objDoc = null;
            ...
            public override void OnEndPage(PdfWriter writer, Document document)
            {
                ...
                Phrase p1Header = new Phrase(String.Format("Transportista"), time);//FontFactory.GetFont("arial", 10));
                Phrase p2Header = new Phrase(String.Format("Patente"), time);//FontFactory.GetFont("arial", 10));
                Phrase p3Header = new Phrase(String.Format("N° Guia"), time);//FontFactory.GetFont("arial", 10));
                Phrase p4Header = new Phrase(String.Format("Fecha"), time);//FontFactory.GetFont("arial", 10));
                Phrase p5Header = new Phrase(String.Format("N° OT"), time);//FontFactory.GetFont("arial", 10));
                Phrase p6Header = new Phrase(String.Format("Nombre OT"), time);
                Phrase p7Header = new Phrase(String.Format("Peso"), time);
                Phrase p8Header = new Phrase(String.Format("Despachado"), time);
                Phrase p9Header = new Phrase(String.Format("Total Kilos"), time);
                ...
                PdfPTable pdfTa = new PdfPTable(9);
                ...
                PdfPCell pdfCell9 = new PdfPCell(p9Header);
                ...
                pdfCell8.BackgroundColor = new Color(165, 165, 165);
                pdfCell9.BackgroundColor = new Color(165, 165, 165);
